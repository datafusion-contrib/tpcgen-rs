...
fn test_tpcgen_cli_tpcds_parquet_matches_single_pass_generation() {
    ...
    assert!(
        num_row_groups > 1,
        "expected store_sales to span multiple source ranges so range-boundary coverage is exercised"
    );
    ...
    assert!(
        num_row_groups > 1,
        "expected store_returns to span multiple sales source ranges so return rows are checked across range boundaries"
    );
...
fn test_tpcgen_cli_tpcds_parquet_num_threads_equivalence() {
    ...
    for num_threads in ["1", "4"] {
        ...
        assert!(
            num_row_groups > 1,
            "expected multiple row groups so --num-threads=4 exercises parallel encoding"
        );
