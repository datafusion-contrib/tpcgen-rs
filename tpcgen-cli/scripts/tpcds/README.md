...
2. **C `dsdgen`** (`--compat c`) — the original TPC-supplied reference
   ...
   match the C reference (see [BUGS.md](../../../tpcdsgen/BUGS.md) and the parent
   [README](../../../tpcdsgen/README.md)).
...
```bash
# Default (MD5-only):
./scripts/tpcds/compare-all-tables.sh
```
...
```bash
./scripts/tpcds/bootstrap-trino.sh                      # first time only
./scripts/tpcds/generate-fixtures.sh                    # produces .dat files
./scripts/tpcds/compare-all-tables.sh --full
```
...
./scripts/tpcds/generate-fixtures.sh --compat c              # sf1
./scripts/tpcds/generate-fixtures.sh --compat c --scale 2    # sf2
./scripts/tpcds/compare-all-tables.sh --compat c --full

# Or compare a single table.
./scripts/tpcds/compare-table.sh reason --compat c                 # MD5-only
./scripts/tpcds/compare-table.sh reason --compat c --full          # byte-for-byte
```
...
### Default (MD5-only)
```bash
./scripts/tpcds/compare-table.sh <table>                # one table, vs. Trino
./scripts/tpcds/compare-all-tables.sh                   # all tables, vs. Trino
./scripts/tpcds/compare-all-tables.sh --compat c        # all tables, vs. C dsdgen
```
...
./scripts/tpcds/generate-fixtures.sh                    # one-time
./scripts/tpcds/compare-all-tables.sh --full
...
no-op for them. At scale factor 10 the six that really split are `store_sales`,
`store_returns`, `catalog_sales`, `catalog_returns`, `inventory` and
`customer_demographics`; the other 18 come out as a single part.

### Cleanup
```bash
./scripts/tpcds/clean-fixtures.sh --yes      # remove all fixtures
```
...
```bash
./scripts/tpcds/generate-fixtures.sh X
```

**Problem:** `Fixture not found` (C path)
```bash
./scripts/tpcds/generate-fixtures.sh --compat c --scale N
```
...
- run: ./scripts/tpcds/compare-all-tables.sh --quiet
...
- run: ./scripts/tpcds/bootstrap-trino.sh
- run: ./scripts/tpcds/generate-fixtures.sh --quiet
- run: ./scripts/tpcds/compare-all-tables.sh --full --quiet

# C dsdgen conformance (--full)
- run: ./scripts/tpcds/generate-fixtures.sh --compat c
- run: ./scripts/tpcds/compare-all-tables.sh --compat c --full --quiet
```
